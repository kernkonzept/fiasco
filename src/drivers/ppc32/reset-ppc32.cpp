...
platform_reset()
{
  ...
  Io::write<Unsigned32>(0, reinterpret_cast<void *>(mbar + Gp0_mode));
  Io::write<Unsigned32>(0xff, reinterpret_cast<void *>(mbar + Gp0_count)); //timeout

  //0x9004 (0x8000 enable watchdog | 0x1000 CE bit - start counter |
  //0x4 CE bit controls timer counter
  Io::write<Unsigned32>(0x9004, reinterpret_cast<void *>(mbar + Gp0_mode));
