...
void Timer::bsp_init(Cpu_number)
{
  const char *c[] = { "arm,armv7-timer", "arm,armv8-timer", "arm,cortex-a15-timer" };
  Dt::Node timer_node = Dt::node_by_compatible_list(c);
  if (!timer_node.is_valid())
    panic("No timer node found in DT");

  Dt::Array_prop<3> ints = timer_node.get_prop_array("interrupts", { 1, 1, 1 });

  if (ints.elements() < 2)
    panic("Timer DT: Generic timer node does not have at least 2 interrupts");

  bool have_interrupt_names = false;
  timer_node.stringlist_for_each("interrupt-names", [&](unsigned idx, const char *n)
    {
      have_interrupt_names = true;
      if (!strcmp(n, "phys"))
        _irq_phys = Dt::get_arm_gic_irq_array(ints, idx);
      else if (!strcmp(n, "virt"))
        _irq_virt = Dt::get_arm_gic_irq_array(ints, idx);
      else if (!strcmp(n, "hyp-phys"))
        _irq_hyp = Dt::get_arm_gic_irq_array(ints, idx);
      // other names: sec-phys, hyp-virt
    });

  if (!have_interrupt_names)
    {
      _irq_phys = Dt::get_arm_gic_irq_array(ints, 0); // Secure EL1
      _irq_virt = Dt::get_arm_gic_irq_array(ints, 1); // Non-Secure EL1

      _irq_hyp = 0;

      if (ints.elements() >= 4)
        _irq_hyp = Dt::get_arm_gic_irq_array(ints, 3); // Non-Secure EL2
    }
