          ...
          if (m)
            {
              ...
              handle_irq<Irq_chip_bcm>(map[cxx::log2u(m)], nullptr);
              continue;
            }
        }

      if (p)
        handle_irq<Irq_chip_bcm>(b + cxx::log2u(p), nullptr);
      else
        return;
    }
}
...
// ------------------------------------------------------------------------
IMPLEMENTATION [arm && pf_rpi && (pf_rpi_rpi2 || pf_rpi_rpi3 || (pf_rpi_rpi4 && !64bit))]:
