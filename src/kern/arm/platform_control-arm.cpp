...
PRIVATE static
void
Platform_control::boot_ap_cpus_acpi(Address phys_tramp_mp_addr)
{
...
PRIVATE static
void
Platform_control::boot_ap_cpus_acpi(Address)
{}

// ------------------------------------------------------------------------
IMPLEMENTATION [arm && mp && (dt || arm_acpi)]:
...
{
  if (Dt::have_fdt())
    boot_ap_cpus_dt(phys_tramp_mp_addr);
  else
    boot_ap_cpus_acpi(phys_tramp_mp_addr);
}
