...
parse_arm_disassembler_option (char *option)
{
  ...
  if (CONST_STRNEQ (option, "reg-names-"))
	...
	printf (_("Unrecognised register name set: %s\n"), option);
    }
    ...
    /* XXX - should break 'option' at following delimiter.  */
    printf (_("Unrecognised disassembler option: %s\n"), option);
...
void
print_arm_disassembler_options (FILE * stream __attribute__((unused)))
{
  int i;
...
the -M switch:\n"));
  ...
  for (i = NUM_ARM_REGNAMES; i--;)
    printf ("  reg-names-%s %*c%s\n",
	     regnames[i].name,
	     (int)(14 - strlen (regnames[i].name)), ' ',
	     regnames[i].description);

  printf ("  force-thumb              Assume all insns are Thumb insns\n");
