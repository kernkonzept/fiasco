...
#if 1 // added for Fiasco

static void *bsearch(const void *key, const void *base, size_t high,
...
static void qsort(void  *base, size_t nel, size_t width,
                  int (*comp)(const void *, const void *))
{
  size_t wnel, gap, wgap;
  wnel = width * nel;
  for (gap = 0; ++gap < nel;)
    gap *= 3;
  while ((gap /= 3) != 0)
		       ...
		       bfd_vma *address, char** notes ATTRIBUTE_UNUSED)
{
  ...
  switch (opnd->type)
	...
	single_conv_t c;
	c.i = opnd->imm.value;
#if 0 // Fiasco: no floating point support
	snprintf (buf, size, "#%.1f", c.f);
#else
	snprintf (buf, size, "<FLOAT>");
#endif // Fiasco: no floating point support
	break;
      }
